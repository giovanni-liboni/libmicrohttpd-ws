                      ...
                      enum MHD_RequestTerminationCode termination_code)
{
  ...
  if( connection->ws_upgraded == MHD_NO )
...
try_ready_normal_body (struct MHD_Connection *connection)
{
  ...
  // set TCP_NODELAY
  val = 1;
  setsockopt (connection->socket_fd, IPPROTO_TCP, TCP_NODELAY, &val, sizeof (val));
...
do_read (struct MHD_Connection *connection)
{
  ...
  if(connection->ws_upgraded == MHD_YES)
	  	  return MHD_YES;
  ...
  if (0 == bytes_read && connection->ws_upgraded == MHD_NO )
    {
