...
{
	if ( connection == NULL ) return MHD_NO;
	...
	connection->read_closed = MHD_NO;
	  ...
	  MHD_connection_close (connection, MHD_REQUEST_TERMINATED_DAEMON_SHUTDOWN );
	  ...
	  if (0 != pthread_mutex_lock(&daemon->cleanup_connection_mutex))
	    {
	      ...
	      DLL_remove (connection->daemon->cleanup_head,
	    		  connection->daemon->cleanup_tail,
	    		  connection);
//	      if ( (0 != (connection->daemon->options & MHD_USE_THREAD_PER_CONNECTION)) &&
//		   (MHD_NO == connection->thread_joined) )
//		{
//	      liblog("Waiting thread...");
//	      pthread_cancel(connection->pid);
//		  if (0 != (rc = pthread_join (connection->pid, &unused)))
//		    {
//		      MHD_PANIC ("Failed to join a thread\n");
//		    }
//		  liblog("OK\n");
//		}
	...
	#endif
	      MHD_ip_limit_del (connection->daemon, (struct sockaddr*)connection->addr, connection->addr_len);
